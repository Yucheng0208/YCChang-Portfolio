...
e.target.classList.add('active');
    ...
    (function setupGradeInquiryPage() {
        ...
        let availableCourses = []; // 儲存從YAML載入的課程
        ...
        function showLoading(show) {
            ...
            studentIdInput.disabled = show;
            courseSelect.disabled = show;
        }
        ...
        // 1. 從 YAML 載入課程列表到下拉選單
        async function loadCourses() {
            try {
                const response = await fetch('courses_for_grades.yaml');
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                const yamlText = await response.text();
                availableCourses = jsyaml.load(yamlText) || [];
                ...
                availableCourses.forEach(course => {
                    ...
                    textSpan.innerHTML = `<span class="lang-en">${course.name.en} (${course.code})</span><span class="lang-zh">${course.name.zh} (${course.code})</span>`;
                    option.appendChild(textSpan);
                    option.textContent = isChinese() ? `${course.name.zh} (${course.code})` : `${course.name.en} (${course.code})`;
        ...
        function updateCourseSelectLanguage() {
            ...
            Array.from(courseSelect.options).forEach((option, index) => {
                 if (option.value === "") { // 預設選項
                    option.textContent = isZh ? "-- 請選擇一門課程 --" : "-- Please select a course --";
                 } else if (availableCourses[index - 1]) { // 其他課程選項
                    const course = availableCourses[index - 1];
                    option.textContent = isZh ? `${course.name.zh} (${course.code})` : `${course.name.en} (${course.code})`;
                }
            });
        }
        ...
        async function performSearch() {
            ...
            if (!courseId) {
                alert(isChinese() ? '請先選擇一門課程。' : 'Please select a course first.');
                return;
            }
            if (!studentId) {
                alert(isChinese() ? '請輸入您的學號。' : 'Please enter your Student ID.');
                return;
            }
            ...
            try {
                ...
                if (!selectedCourse) throw new Error('Course configuration not found.');
        ...
        function processGradeData(data, studentId, courseInfo) {
            const config = {};
            const configRows = data.slice(0, 5);
            ...
            configRows.forEach(row => {
                ...
                if (type) {
                    headers.forEach(header => {
                        if (header !== 'ID') {
                            config[header][type] = row[header];
                        }
                    });
                }
            });
    
            const studentData = data.find(row => row.ID && row.ID.toUpperCase() === studentId);
        ...
        function renderResults(student, config, courseInfo) {
            ...
            Object.keys(student).forEach(key => {
                ...
                const displayName = isZh ? config[key].display_name_zh : config[key].display_name_en;
