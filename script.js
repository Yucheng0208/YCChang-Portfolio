document.addEventListener('DOMContentLoaded', function () {
    ...
    if (swiperElement) {
        new Swiper('.swiper', { loop: true, autoplay: { delay: 4000, disableOnInteraction: false }, pagination: { el: '.swiper-pagination', clickable: true }, navigation: { nextEl: '.swiper-button-next', prevEl: '.swiper-button-prev' }, });
    }
        ...
        if (link) {
            ...
            if (li.classList.contains('nav-item-dropdown')) {
                ...
                subLinks.forEach(subLink => {
                    const subLinkPath = subLink.getAttribute('href').split('/').pop();
    ...
    navList.addEventListener('mouseover', (e) => {
        ...
        if (activeLi) {
            ...
            if (activeLi !== targetLi) { activeLi.classList.add('temporarily-hidden'); }
        }
    });
    navList.addEventListener('mouseout', () => {
        const activeLi = document.querySelector('.nav-links li.active-link.temporarily-hidden');
        if (activeLi) { activeLi.classList.remove('temporarily-hidden'); }
    });
